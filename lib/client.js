...
function SufiaClient(options, sync) {
  ...
  options = options || {};
  options.base = options.base || SufiaClient.DEFAULT_HOST;

  extend(options, {
    create: join(options.base, '/api/items'),
    update: join(options.base, '/api/items/:id'),
    delete: join(options.base, '/api/items/:id')
  });
...
SufiaClient.CONTENT_TYPES = [
  'application/pdf'
];

SufiaClient.getSuitableAttachment = function (items) {
  ...
  for (i = 0, ii = items.length; i < ii; ++i) {
    ...
    if (SufiaClient.CONTENT_TYPES.indexOf(next.data.contentType) < 0)
      continue;
 ...
 * @return {Promise}
 */
SufiaClient.prototype.delete = function (id) {
  return this
...
SufiaClient.prototype.create = function (key, item) {
  return this
    ...
    .tap(function (response) {
      this.register(key, response.header.location);
    }.bind(this));
};
...
SufiaClient.prototype.update = function (key, item) {
  ...
  return (id)
    ? this.send('put', this.path('update', { id: id }), null, item)
...
/**
 * Saves a Zotero item key to Sufia id mapping,
 * extracted from the given URL.
 *
 * @method register
 */
SufiaClient.prototype.register = function (key, url) {
  var m = (/api\items\/([^\/]+)/).exec(url || '');
  if (m) this.sync.subscription.data.sufia[key] = m[1];
...
SufiaClient.prototype.collect = co(function* (keys) {
  ...
  for (i = 0, ii = keys.length; i < ii; ++i) {
    try {
      ...
      child = SufiaClient.getSuitableAttachment(item.children);

      if (!child) {
        this.debug('skipping "%s": item has no suitable attachments', item.key);
        continue;
      }
