 ...
 * @class SufiaItem
 */
function SufiaItem(token, zotero, child, data) {
  this.token    = token;
  this.file     = {};
  this.metadata = {
  ...
  if (child) this.attach(child, data);
}
...
SufiaItem.prototype.parse = function (z) {
  ...
  this.metadata.rights = z.data.rights || this.metadata.rights;

  if (z.data.place)
    this.metadata.basedNear = z.data.place;

  if (z.data.abstractNote)
    this.metadata.description = z.data.abstractNote;

  var identifier = z.data.DOI || z.data.ISBN || z.data.PMID || z.data.arXiv;
